Session.setDefault('statusesSearchFilter', '');
...
Template.statusesListPage.events({
  'click .fa-sun-o':function(){
    Router.go('/insert/status');
  },
  'click .fooItem':function(){
    Router.go('/view/status/' + this._id);
  },
  // use keyup to implement dynamic filtering
  // keyup is preferred to keypress because of end-of-line issues
  'keyup #recordSearchInput': function() {
    Session.set('statusesSearchFilter', $('#recordSearchInput').val());
  }
});
...
Template.statusesListPage.rendered = function(){
...
Template.statusesListPage.helpers({
  ...
  statusesList: function() {
    ...
    return Statuses.find({
      title: {
        $regex: Session.get('statusesSearchFilter'),
        $options: 'i'
    }});
  }
});
